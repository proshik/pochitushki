...
- **Экспорт**: выгрузите свои данные в CSV.
- **Два языка интерфейса**: русский и английский (определяется по языку Telegram).
...
входит. Чтобы собрать образ с Playwright-движком:
...
> `exp` (не истёк), `aud` (совпадает с `client_id`) и `iss` (выдан Telegram).
