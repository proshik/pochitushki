...
- Судьба `PostData.tags` (плоский `TEXT[]` из Pocket-импорта): сейчас рядом с
  метками живёт вторая, доступная только на чтение система тегов. Слить их
  (импорт → метки + миграция существующих строк) — отдельное решение
