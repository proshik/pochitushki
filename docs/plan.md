...
### Доделано после (тот же объём, вторым заходом)

- [x] **Веб-UI**: пикер на карточке (тумблеры + «создать и повесить») и страница
      `/labels` — счётчики по обеим полкам, инлайн-переименование, удаление с
      подтверждением, создание. Чипы на карточках — ссылки в `/labels/{id}`
- [x] **Бот**: кнопка «🏷 Метки» на карточке поста, тумблеры и «➕ Новая метка»
      (ждёт следующее сообщение через `PendingLabelInput`, in-memory, TTL 5 мин)
- [x] **Сентинел** для выборки по метке: `post-list` больше не собирает URL сам,
      а получает готовый от контроллера — иначе фильтр дотягивал посты без метки
- [x] `PATCH /api/v1/labels/{id}` — переименование; конфликт имени = 409, метки
      не сливаются молча
